document.addEventListener('DOMContentLoaded',function(){
    ...
    var mobileMenuMQ=window.matchMedia('(max-width:991.98px)');//para detectar cuando cmb entre mob-dsk
    //Menu mobile 
    if(headerEl){
    ...
    //submenus del header //estado para poder activar/desactivar listener y clases en submenus mobile
    ...
    function updateSubmenuMode(){
      if(mobileMenuMQ.matches)enableMobileSubmenus();
      else{
    ...
    updateSubmenuMode();
    if(mobileMenuMQ.addEventListener){
      mobileMenuMQ.addEventListener('change',updateSubmenuMode);
    }else{
      mobileMenuMQ.addListener(updateSubmenuMode);
    }
});